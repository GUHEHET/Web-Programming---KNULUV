...
<script>
	...
	function file_upload(email){
		var file = document.getElementById('fileItem').files[0];
		console.log(email.split('@')[0]);
		var uploadRef = storage.child('img/'+email.split('@')[0]+'.png');
				...
				<form class="login-form">
					...
					<div class="form-group">
						<label class="text-uppercase">사진</label> <input type="file"
							class="form-control" value="파일선택" id="fileItem" placeholder="">
					</div>

					<button type="button" class="btn btn-login margin "
						onclick="signup(email.value, pwd.value, pwd_check.value, Nickname.value, age.value, sex.value, department.value, text.value); file_upload(email.value);">SIGN
						UP</button>
				</form>
			</div>
		</div>
